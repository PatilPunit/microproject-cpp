...
void Pokemon::level_up(Pokemon &pokem) {
    ...
    if(pokem.exp >= pokem.max_exp) {
        ...
        cout << "Your Pokemon is having" <<pokem.hp<< " HP" << endl;
        check_evolve(pokem);
    }
}
...
protected:
    ...
    struct Items {
        int pokeball = 10, preball = 0, masterball = 0, hball = 0;
...
public:
    ...
    /*Pokemon*/void catchPokemon(Pokemon &player); //
    void bag();// in gym may b
    void detail();//
    void store();
    Pokemon searchPokemon(Pokemon& player);//
    void catchPokemonDuringBattle(Pokemon& wildPokemon);//
    void switchPokemon();// used in gym class
    void Pokemon_center();// in gym class
    void city();// in gym class
};
void Game::battle(Pokemon &player, Pokemon &wild) {
    while(true){
        Pokemon &player = catchedPokemons[activePokemonIndex];
    if (player.isFainted()) {
        cout << player.name << " has fainted and cannot battle!\n";
        switchPokemon();
        player = catchedPokemons[activePokemonIndex];
        if (player.isFainted()) {
            cout << "No healthy Pokémon left... You lost!\n";
            return;
        }
        continue;
    }
    ...
    while (!player.isFainted() && !wild.isFainted()) {
        cout << "\n" << player.name << "'s HP: " << player.hp << " | " << wild.name << "'s HP: " << wild.hp << endl;
        cout << "1. Fight\n2. Bag\n3. Change Pokémon\n4. Run\n5. Catch\n";
        int choice;
        cin >> choice;
        switch (choice) {
        case 1: {
            ...
            for (int i = 0; i < player.moves.size(); i++) {
                cout << i + 1 << ". " << player.moves[i].name << " (Damage: " << player.moves[i].damage << ")\n";
            }
            int moveChoice;
            cout << "Choose move: ";
            cin >> moveChoice;
...
/*Pokemon*/void Game::catchPokemon(Pokemon &player) {
...
void Game :: menu() {
    ...
    int start;
    cin >> start;
    Pokemon starter = allPokemons[start - 1];
    catchedPokemons.push_back(starter);
    ...
    while(1) {
        ...
        case 1:
            ...
            break;
        case 2:
            catchPokemon(catchedPokemons[activePokemonIndex]);
            break;
        case 3:
            bag();
            break;
    ...
    while(u!=6) {
        ...
        if(u==2) {
            ...
            if(i.boulder==0&&i.fire==0&&i.rainbow==0&&i.earth==0&&i.marsh==0&&i.cascade==0&&i.thunder==0&&i.soul==0) {
                cout << "No badges yet go to gyms" << endl;
            }
        }
        if(u==3) {
            if(i.hpotion>0) {
        ...
        if(u==4) {
            ...
            if(i.megastone==0&&i.firestone==0&&i.waterstone==0&&i.grasstone==0&&i.thunderstone==0) {
                cout << "You don't have any stones" << endl;
            }
        }
        if(u==5) {
            return;
...
class Gym : public Game {
public:
    virtual void gymBattle(Pokemon &player) = 0;
    void gymDetails();
};
class BrockGym : virtual public Gym {
protected:
    vector<Pokemon> brockPokemons;
public:
    BrockGym() {
        brockPokemons.push_back(gymPokemon[0]);
        brockPokemons.push_back(gymPokemon[1]);
    }
    void gymBattle(Pokemon &player) override {
        cout << "💪 Brock challenges you!\n";
        for (size_t i = 0; i < brockPokemons.size(); i++) {
            Pokemon &gPoke = brockPokemons[i];
            cout << "Brock sends out " << gPoke.name << "!\n";
            battle(player, gPoke);
            if (player.isFainted()) {
                cout << "You lost to Brock... Try again!\n";
                return;
            }
        }
        cout << "🎉 You defeated Gym Leader Brock and earned the Boulder Badge!\n";
        i.boulder = 1;
    }
};
class MistyGym : virtual public Gym {
protected:
    vector<Pokemon> mistyPokemons;
public:
    MistyGym() {
        mistyPokemons.push_back(gymPokemon[2]);
        mistyPokemons.push_back(gymPokemon[3]);
    }
    void gymBattle(Pokemon &player) override {
        cout << "💧 Misty challenges you!\n";
        for (size_t i = 0; i < mistyPokemons.size(); i++) {
            Pokemon &gPoke = mistyPokemons[i];
            cout << "Misty sends out " << gPoke.name << "!\n";
            battle(player, gPoke);
            if (player.isFainted()) {
                cout << "You lost to Misty... Train harder!\n";
                return;
            }
        }
        cout << "🎉 You defeated Gym Leader Misty and earned the Cascade Badge!\n";
        i.cascade = 1;
    }
};
class LtSurgeGym : virtual public Gym {
protected:
    vector<Pokemon> surgePokemons;
public:
    LtSurgeGym() {
        surgePokemons.push_back(gymPokemon[4]);
        surgePokemons.push_back(gymPokemon[5]);
    }
    void gymBattle(Pokemon &player) override {
        cout << "⚡ Lt. Surge challenges you!\n";
        for (size_t i = 0; i < surgePokemons.size(); i++) {
            Pokemon &gPoke = surgePokemons[i];
            cout << "Lt. Surge sends out " << gPoke.name << "!\n";
            battle(player, gPoke);
            if (player.isFainted()) {
                cout << "You lost to Lt. Surge... Zap! ⚡\n";
                return;
            }
        }
        cout << "🎉 You defeated Lt. Surge and earned the Thunder Badge!\n";
        i.thunder = 1;
    }
};
class ErikaGym : virtual public Gym {
protected:
    vector<Pokemon> erikaPokemons;
public:
    ErikaGym() {
        erikaPokemons.push_back(gymPokemon[6]);
        erikaPokemons.push_back(gymPokemon[7]);
        erikaPokemons.push_back(gymPokemon[8]);
    }
    void gymBattle(Pokemon &player) override {
        cout << "🌿 Erika challenges you!\n";
        for (size_t i = 0; i < erikaPokemons.size(); i++) {
            Pokemon &gPoke = erikaPokemons[i];
            cout << "Erika sends out " << gPoke.name << "!\n";
            battle(player, gPoke);
            if (player.isFainted()) {
                cout << "You lost to Erika... Smell the flowers 🌸 and try again!\n";
                return;
            }
        }
        cout << "🎉 You defeated Erika and earned the Rainbow Badge!\n";
        i.rainbow = 1;
    }
};
void Game::city() {
    ...
    cout << "1. Pewter City (Gym Leader: Brock – Rock type) 🪨\n";
    cout << "2. Cerulean City (Gym Leader: Misty – Water type) 💧\n";
    cout << "3. Vermilion City (Gym Leader: Lt. Surge – Electric type) ⚡\n";
    cout << "4. Celadon City (Gym Leader: Erika – Grass type) 🌿\n";
    cin >> cityChoice;
    cout << "\nYou arrived in ";
    switch (cityChoice) {
        case 1: cout << "Pewter City 🪨!\n"; break;
        case 2: cout << "Cerulean City 💧!\n"; break;
        case 3: cout << "Vermilion City ⚡!\n"; break;
        case 4: cout << "Celadon City 🌿!\n"; break;
        default: cout << "an unknown place... 😅\n"; return;
    }
    int actionChoice;
    cout << "Where would you like to go?\n";
    cout << "1. Pokémon Center 🏥\n";
    cout << "2. Gym 🏛️\n";
    cin >> actionChoice;
    if (actionChoice == 1) {
        if (catchedPokemons.empty()) {
            cout << "You don’t have any Pokémon to heal!\n";
        } else {
            for (size_t i = 0; i < catchedPokemons.size(); i++) {
                catchedPokemons[i].hp = catchedPokemons[i].max_hp;
            }
            cout << "All your Pokémon have been healed at the Pokémon Center! 🏥✨\n";
        }
    }
    else if (actionChoice == 2) {
        if (catchedPokemons.empty()) {
            cout << "You don’t have any Pokémon to battle with!\n";
            return;
        }
        Pokemon &player = catchedPokemons[activePokemonIndex];
        switch (cityChoice) {
            case 1: {
                BrockGym brock;
                brock.gymBattle(player);
                break;
            }
            case 2: {
                MistyGym misty;
                misty.gymBattle(player);
                break;
            }
            case 3: {
                LtSurgeGym surge;
                surge.gymBattle(player);
                break;
            }
            case 4: {
                ErikaGym erika;
                erika.gymBattle(player);
                break;
            }
            default:
                cout << "This city doesn’t have a Gym yet!\n";
        }
    }
    else {
        cout << "Invalid choice.\n";
